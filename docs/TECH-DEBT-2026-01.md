# Brain 專案技術債報告

> 建立日期：2026-01-19
> 專案：Brain - AI 客服系統（LINE 訊息平台）

---

## 📊 專案概況

| 指標 | 數值 |
|------|------|
| 後端 Python 檔案 | 73 個 |
| API 路由 | 17 個 |
| 測試檔案 | 15 個 |
| 整體健康分數 | 6.7/10 |

### 技術棧

- **後端**：FastAPI + SQLAlchemy + PostgreSQL
- **前端**：React 18 + Vite + Tailwind CSS
- **LLM**：Claude (Anthropic) + Gemini (Google) via OpenRouter
- **訊息平台**：LINE Bot SDK
- **儲存**：Cloudflare R2（照片）

---

## 🔴 P0 - 嚴重問題（必須優先處理）

### 1. CRM 客戶端重複

**檔案**：
- `backend/services/jungle_client.py`（465 行）
- `backend/services/crm_client.py`（178 行）

**問題描述**：
兩個客戶端功能部分重疊，但實作方式不同：

| 功能 | jungle_client | crm_client |
|------|---------------|------------|
| 客戶查詢 | ✅ 直接 PostgREST | ❌ 無 |
| 合約查詢 | ✅ 直接 PostgREST | ❌ 無 |
| 繳費查詢 | ✅ 直接 PostgREST | ❌ 無 |
| MCP 工具調用 | 部分（create_lead） | ✅ 通用 call_tool |
| 報價單建立 | ❌ 無 | ✅ 有 |
| 錯誤處理 | print() | logger |

**使用位置**：
```
jungle_client (4 處)：
├── brain/draft_generator.py
├── services/booking_handler.py
├── api/routes/analysis.py
└── api/routes/webhooks.py

crm_client (1 處)：
└── api/routes/quotes.py
```

**風險**：
- 維護兩套程式碼
- 錯誤處理不一致
- 新功能不知道該加在哪

**建議方案**：
1. 保留 `crm_client.py` 作為主要客戶端
2. 將 `jungle_client.py` 的客戶/合約/繳費查詢功能遷移到 `crm_client.py`
3. 更新所有引用
4. 移除 `jungle_client.py`

---

### 2. ~~Intent 分類重複~~ ✅ 2026-01-20 完成（死代碼清理）

**原檔案**：
- `backend/brain/router.py` - JSON 模式（已刪除）
- `backend/services/knowledge_service.py` - DB 模式

**解決**：
經分析發現兩個 `classify_intent()` 都是死代碼，實際 routing 由 `claude_client.route_task()` 執行。
- 刪除 `brain/router.py`（181 行）
- 刪除 `tests/test_router.py`
- 移除 `knowledge_service.py` 中的 `classify_intent()` 方法
- 移除 `draft_generator.py` 中未使用的 `intent_router` 引用

---

### 3. ~~Flex Message 模板重複~~ ✅ 2026-01-20 完成（死代碼清理）

**原檔案**：
- `backend/brain/flex_templates.py`（已刪除）
- `backend/services/flex_templates.py`（保留）

**解決**：
`brain/flex_templates.py` 是未被引用的死代碼，直接刪除。

---

## 🟡 P1 - 中等問題

### 4. 型別提示不完整

**現況**：
| 模組 | 覆蓋率 |
|------|--------|
| services/ | ~60% |
| brain/ | ~47% |
| api/routes/ | ~50% |

**影響的檔案**：
- `brain/draft_generator.py` - 返回型別不明確
- `services/rag_service.py` - 搜尋結果型別不明確

**建議**：建立 `types.py` 共用型別定義。

---

### 5. N+1 查詢問題

**位置**：`backend/api/routes/messages.py` 的 `list_conversations()`

**問題**：在迴圈內查詢每個對話的最後訊息。

**建議**：使用 JOIN 或批次查詢。

---

### 6. 錯誤處理不一致

**現況**：
- 部分模組用 `print()` 輸出錯誤
- 部分模組用 `logger`

**建議**：統一使用 `logging` 模組。

---

## 🟢 P2 - 可改進項目

### 7. Prompt 管理

**現況**：Prompt 硬編碼在 `backend/brain/prompts.py`，無版本控制。

**建議**：建立 Prompt 版本管理系統（DB 驅動）。

---

### 8. 知識庫管理

**現況**：`logic_tree.json`、`sales_mindmap.json` 等 JSON 檔案在啟動時載入。

**建議**：遷移到資料庫，支援動態更新。

---

## ✅ 處理優先順序

| 順序 | 項目 | 工作量 | 風險 | 狀態 |
|------|------|--------|------|------|
| 1 | 合併 CRM 客戶端 | M | 低 | ✅ 2026-01-20 完成 |
| 2 | 移除重複 Flex 模板 | S | 低 | ✅ 2026-01-20 完成（刪除死代碼） |
| 3 | 清理 Intent 分類死代碼 | S | 低 | ✅ 2026-01-20 完成 |
| 4 | 補齊型別提示 | L | 低 | ⏳ |
| 5 | 修復 N+1 查詢 | S | 低 | ⏳ |
| 6 | 統一錯誤處理 | M | 低 | ⏳ |
| 7 | Prompt 版本管理 | L | 中 | ⏳ |
| 8 | 知識庫 DB 化 | L | 高 | ⏳ |

---

## 📁 關鍵檔案清單

| 類別 | 檔案 | 行數 | 說明 |
|------|------|------|------|
| CRM | services/crm_client.py | ~600 | 統一 CRM 客戶端（整合 PostgREST + MCP Tools） |
| 知識 | services/knowledge_service.py | ~300 | 知識庫服務（意圖樹、SPIN 問題） |
| 模板 | services/flex_templates.py | ~310 | Flex 模板（照片展示） |
| 核心 | brain/draft_generator.py | ~400 | LLM 回覆生成 |
| API | api/routes/messages.py | 653 | 訊息 API（有 N+1） |

---

## 🔧 測試覆蓋率

**目前**：15 個測試檔案，主要測試 `draft_generator.py`

**需要補充**：
- RAG Service 測試
- Router 測試
- 成本計算測試
- API 整合測試

---

*此報告由 Claude Code 自動生成，基於 2026-01-19 的程式碼分析。*
