...
async def analyze_conversation_for_quote(
    ...
    try:
        ...
        logger.info(f"[QuoteAnalyze] Total messages: {len(messages)}")
        for m in messages[:5]:
            logger.info(f"[QuoteAnalyze] Message {m.id}: source={m.source}, sender_name={m.sender_name}")

        customer_msg = next(
            (m for m in messages if m.source not in ('line_bot', 'system')),
            None
        )
        logger.info(f"[QuoteAnalyze] customer_msg: {customer_msg.id if customer_msg else None}, source={customer_msg.source if customer_msg else None}, name={customer_msg.sender_name if customer_msg else None}")
